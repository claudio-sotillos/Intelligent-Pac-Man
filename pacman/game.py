...
class Game:
    ...
    def run( self ):
        ...
        while not self.gameOver:
            ...
            if agentIndex ==0:
                tick = agent.countActions

                if tick == 0:
                    # print(agent.mytuple(self.state, self.gameOver, printlist))
                    agent.mytuple(self.state, self.gameOver, printlist)
                else:
                    # print(agent.mytuple(self.state, self.gameOver, printlist))
                    update_tuple =  agent.mytuple(self.state, self.gameOver, printlist)
                    st = update_tuple[0]
                    act = update_tuple[1]
                    nxt_st = update_tuple[2]
                    re = update_tuple[3]
                    agent.update(st,act,nxt_st,re)
